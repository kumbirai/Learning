...
//: za.co.coach.learning.tij.containers/ListPerformance.java
...
import za.co.coach.util.CountingIntegerList;
	...
	static {
		...
		tests.add(new Test<List<Integer>>("insert") {
			int test(List<Integer> list, TestParam tp) {
				int loops = tp.loops;
				for (int i = 0; i < loops; i++)
					list.add(5, 47); // Minimize random-za.co.coach.learning.tij.access cost
				return loops;
			}
		});
		tests.add(new Test<List<Integer>>("remove") {
			int test(List<Integer> list, TestParam tp) {
				...
				for (int i = 0; i < loops; i++) {
					...
					while (list.size() > 5)
						list.remove(5); // Minimize random-za.co.coach.learning.tij.access cost
				}
				return loops * size;
			}
		});
